...
export function buildEventReviewSummary({
  ...
  const suggestedNextTest = problemMatchup
    ? `Play a 5-game testing block with ${deckLabel} into ${problemMatchup}, especially tracking turn order and first issue tags.`
    : consistencyIssue
      ? `Run a 5-game testing block with ${deckLabel} focused on opening consistency and sequencing.`
      : rounds.length
        ? `Repeat ${deckLabel} into the most common matchup from this event and keep tagging what breaks first.`
