...
async function inspectCounts(userId) {
  ...
  for (const table of ["decks", "matches", "matchup_notes"]) {
...
const DECK_BLUEPRINTS = [
  {
    key: "ragingBolt",
    name: "Playtest 200 - Raging Bolt Lab",
  ...
  {
    key: "dragapult",
    name: "Playtest 200 - Dragapult Dusknoir",
    archetype: "Dragapult",
    versions: [
      {
        key: "v1_bare",
  ...
  {
    key: "greninja",
    name: "Playtest 200 - Mega Greninja Tempo",
...
# Cleaner support counts around the Frogadier stage`,
  ...
  {
    key: "charizard",
    name: "Playtest 200 - Charizard Pressure",
    archetype: "Charizard",
    versions: [
      {
        key: "v1_straight",
...
# Slightly more respect for Item Lock and going-first races`,
  ...
  {
    key: "controlLab",
    name: "Playtest 200 - Control Counterlab",
    archetype: "Gholdengo",
    versions: [
      {
        key: "v1_counter",
...
function generateSeedMatches(userId, created) {
  ...
  buildSeries({
    ...
    opponent: "Mega Greninja",
    games: 8,
    resultPattern: (i) => (i < 2 ? "win" : i < 5 ? "loss" : i === 5 ? "tie" : "loss"),
  ...
  buildSeries({
    ...
    games: 8,
    resultPattern: (i) => (i < 3 ? "loss" : i === 3 ? "tie" : "win"),
  ...
  buildSeries({
    ...
    games: 8,
    resultPattern: (i) => (i < 5 ? "win" : i < 7 ? "loss" : "tie"),
    turnPattern: (i) => (i % 2 === 0),
    qualityProfile: stableProfile,
  ...
  buildSeries({
    ...
    games: 6,
    resultPattern: (i) => (i < 3 ? "win" : i === 3 ? "tie" : "loss"),
    turnPattern: () => null,
  ...
  buildSeries({
    ...
    games: 6,
    resultPattern: (i) => (i < 3 ? "win" : i < 5 ? "loss" : "tie"),
  ...
  buildSeries({
    ...
    forcedPositiveFn: (result) => (result === "win" ? ["key tech mattered"] : []),
    cardsFn: (result) => ({
      cardsShined: result === "win" ? ["Manaphy"] : [],
      cardsFailed: result === "loss" ? ["Prime Catcher"] : [],
    }),
    contextFn: () => ({
      gameContext: "testing",
      testingSessionName: "RB anti-bench follow-up",
      focusMatchup: "Mega Greninja",
    }),
  });
  ...
  buildSeries({
    ...
    forcedPositiveFn: (result) => (result === "win" ? ["strong setup"] : []),
    cardsFn: () => ({ cardsShined: [], cardsFailed: [] }),
    contextFn: () => ({ gameContext: "testing", testingSessionName: "RB spread volume" }),
  });

  // Dragapult - 40 matches total
  buildSeries({
    deckKey: "dragapult",
    versionKey: "v1_bare",
    opponent: "Raging Bolt",
    games: 12,
    resultPattern: (i) => (i < 6 ? "loss" : i === 6 ? "tie" : "win"),
    turnPattern: (i) => (i % 3 === 0 ? null : i % 2 === 0),
  ...
  buildSeries({
    ...
    opponent: "Charizard ex",
    games: 8,
    resultPattern: (i) => (i < 4 ? "win" : i < 6 ? "loss" : i === 6 ? "tie" : "win"),
  ...
  buildSeries({
    ...
    opponent: "Rogue decks",
    games: 10,
    resultPattern: (i) => (i < 4 ? "win" : i < 7 ? "loss" : i === 7 ? "tie" : "win"),
    turnPattern: (i) => (i % 4 === 0 ? null : i % 2 === 0),
    qualityProfile: stableProfile,
    forcedIssueFn: (result) => (result === "loss" ? ["prize map error"] : []),
    forcedPositiveFn: (result) => (result === "win" ? ["Good Tech"] : []),
  ...
  // Mega Greninja - 32 matches total
  buildSeries({
    deckKey: "greninja",
    versionKey: "v1_raw",
    opponent: "Raging Bolt",
    games: 14,
    resultPattern: (i) => (i < 9 ? "win" : i < 12 ? "loss" : i === 12 ? "tie" : "win"),
  ...
  buildSeries({
    ...
    forcedPositiveFn: (result) => (result === "win" ? ["Good Tech"] : []),
    cardsFn: () => ({ cardsShined: ["Prime Catcher"], cardsFailed: [] }),
    contextFn: () => ({ gameContext: "testing", testingSessionName: "Greninja side reps" }),
  });

  // Charizard - 32 matches total with visible turn-order signal
  buildSeries({
    deckKey: "charizard",
  ...
  buildSeries({
    ...
    forcedPositiveFn: (result) => (result === "win" ? ["good prize plan"] : []),
    cardsFn: () => ({ cardsShined: [], cardsFailed: [] }),
    contextFn: () => ({ gameContext: "testing", testingSessionName: "Zard rogue cleanup" }),
  });
  ...
  // Control Counterlab - 14 matches total, strong Item Lock loss tag signal
  buildSeries({
    deckKey: "controlLab",
  ...
  if (records.length !== 200) {
    throw new Error(`Expected 200 generated matches, received ${records.length}.`);
  }
...
function buildIntegrityReport({
  ...
  return `# Playtest 200 Integrity Report
...
- Raging Bolt vs Mega Greninja is a real leak with a 5-19-2 record across 26 games.
...
- Multiple versions should show meaningful version signal, especially Raging Bolt v3 and Dragapult v2.
`;
}
...
async function main() {
  ...
  console.log("\n=== SixPrizer 200-log playtest seed ===\n");
  ...
  writeFile(
    "docs/playtest_200_preclean_summary.md",
    `# Playtest 200 Pre-clean Summary
...
Only rows owned by this dedicated test user will be deleted or recreated.
`
  );
  ...
  writeFile(
    "docs/playtest_200_cleanup_report.md",
    `# Playtest 200 Cleanup Report
...
All deletes were scoped to user_id ${user.id}. The auth user row itself was not touched.
`
  );
  ...
  writeFile(
    "results/playtest_200_seeded_games.tsv",
    buildSeededGamesTsv(insertedRows)
  );
  writeFile("results/playtest_200_summary.tsv", buildSummaryTsv(summary));
  writeFile(
    "results/playtest_200_integrity_report.md",
